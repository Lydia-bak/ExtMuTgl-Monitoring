...
async def game_loop():
    ...
    if save_timewise_result_csv:        
        to_save = [
            ...
            # 'remaining_tasks', 
            # 'tasks_total_amount_left'
            ]+["position "+str(i) for i in range(len(agents))]
        # print(len(to_save))
        # print(len(data_records[0]))
        # print(len(data_records[1]))
        # print(len(data_records[2]))
        # print(len(data_records[3]))
        csv_file_path = result_saver.save_to_csv("timewise", data_records, to_save)          

...
open Types
  ...
  type t = 
    ...
    | Exists of agent * t
    | Forall of agent * t
    | Leq of Aformula.t * Aformula.t
    | Eq of Aformula.t * Aformula.t
    | F of float * float * t
  ...
  let rec hSize (phi : t) : float = match phi with
    ...
    | Leq (phi1, phi2) | Eq (phi1, phi2) -> max(Aformula.hSize phi1) (Aformula.hSize phi2)
  ...
  let rec setAgentFormula (a) (b) (phi)  : t = match phi with
    ...
    | Leq (phi1, phi2) -> Leq (Aformula.setAgentFormula a b phi1, Aformula.setAgentFormula a b phi2)
    | Eq (phi1, phi2) -> Eq (Aformula.setAgentFormula a b phi1, Aformula.setAgentFormula a b phi2)
  ...
  *)
  ...
  let rec bSem (phi : t) (aSet : AgentSet.t) (osig : graph signal) (x_: uncertainSet signal signal ref): Bool3.t signal signal = 
    ...
    |Not phi1 -> 
      ...
      let x = bSem phi1 aSet osig x_ in
      FloatMap.map( 
        fun tmap -> (
          FloatMap.map
            (function 
    ...
    | Or (phi1,phi2) ->
        let x = bSem phi1 aSet osig x_ in
        let y = bSem phi2 aSet osig x_ in
    ...
    |And (phi1,phi2) -> bSem (Not (Or(Not phi1, Not phi2))) aSet osig x_


    |Implies(phi1,phi2) -> bSem(Or(Not phi1, phi2)) aSet osig x_


    |Exists (a,phi') ->
      let aa = AgentSet.fold(
          fun s acc -> AgentMap.add s ((fun b ->  bSem (setAgentFormula a b phi') aSet osig x_) s) acc 
    ...
    |Forall (a,phi') -> bSem (Not(Exists(a,Not phi'))) aSet osig x_




    |Leq (phi1,phi2) -> 
      let x = Aformula.setSem phi1 aSet osig x_ Float.infinity in
      let y = Aformula.setSem phi2 aSet osig x_ Float.infinity in
    ...
    |Eq (phi1,phi2) -> 
      bSem(And (Leq (phi1,phi2), Leq (phi2,phi1))) aSet osig x_
      

    |F (a, b, phi') -> 
      let x = bSem phi' aSet osig x_ in
